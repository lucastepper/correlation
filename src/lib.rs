...
// fn corr_single(&data1, &data2) {
//     let index = get_index(cond_data[i], start, dx, nbins);
//     self._conditional_sums[[index, tcorr]] += corr_data1[i] * corr_data2[i + tcorr];
//     self._conditional_counts[[index, tcorr]] += 1;
// }
...
impl ConditionalCorrelation {
    ...
    fn new(py: Python<'_>, bins: Py<np::PyArray1<f64>>, ncorr: usize) -> PyResult<Self> {
        ...
        Ok(Self{ bins, ncorr, _conditional_sums, _conditional_counts })
    }
    ...
    fn add_data<'py> (
        ...
        if corr_data1.len() != corr_data2.len() {
            return Err(PyErr::new::<PyValueError, _>("corr_data1 and corr_data2 must be the same length"));
        }
        if cond_data.len() != corr_data1.len() {
            return Err(PyErr::new::<PyValueError, _>("cond_data and corr_data must be the same length"));
        }
        ...
        self._conditional_sums.axis_iter_mut(nd::Axis(1))
            .into_par_iter()
            .zip(
                self._conditional_counts.axis_iter_mut(nd::Axis(1))
                .into_par_iter()
            )
            .enumerate()
    ...
    fn get_results<'py> (&self, py: Python<'py>) -> PyResult<&'py np::PyArray2<f64>> {
        Ok((self._conditional_counts.mapv(|c| 1. / c as f64 ) * &self._conditional_sums).into_pyarray(py))
    }
}
